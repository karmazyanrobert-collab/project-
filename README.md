...
4. Нажмите **Deploy**.
5. Первый старт может занять больше времени, потому что приложение создаёт `data/`, CSV, Parquet-слои, Feature Store, DuckDB warehouse и небольшую порцию streaming demo data автоматически.
...
В Streamlit Community Cloud отдельный процесс запускать не нужно: dashboard сам создаёт `streaming_events` и `streaming_metrics` при первом открытии, а новую порцию событий можно добавить кнопкой **Generate streaming demo data** в sidebar.

Локально или в Codespaces команда выше запускает live-режим: producer пишет события в `data/events/event_stream.jsonl`, consumer обновляет таблицы `streaming_events` и `streaming_metrics` в DuckDB. После этого обновите dashboard.
